...
export function RankingReviewItemCard({
  ...
  const correctionFields = (
    <div className="grid gap-3 sm:grid-cols-2">
      <div className="space-y-1.5 rounded-2xl border border-border/40 bg-muted/10 p-3">
        <p className="text-xs  text-muted-foreground opacity-80">Governor Game ID</p>
        <Input
      ...
      <div className="space-y-1.5 rounded-2xl border border-border/40 bg-muted/10 p-3">
        <p className="text-xs  text-muted-foreground opacity-80">Alias For Create Alias</p>
        <Input
      ...
      <div className="space-y-1.5 rounded-2xl border border-border/40 bg-muted/10 p-3">
        <p className="text-xs  text-muted-foreground opacity-80">Corrected Rank</p>
        <Input
      ...
      <div className="space-y-1.5 rounded-2xl border border-border/40 bg-muted/10 p-3">
        <p className="text-xs  text-muted-foreground opacity-80">Corrected Name</p>
        <Input
      ...
      <div className="space-y-1.5 rounded-2xl border border-border/40 bg-muted/10 p-3 sm:col-span-2">
        <p className="text-xs  text-muted-foreground opacity-80">Corrected Metric</p>
        <Input
  ...
  return (
    <article className="rounded-[20px] bg-black border border-white/10 shadow-2xl shadow-black/80 backdrop-blur-xl p-3 min-[390px]:rounded-[22px] min-[390px]:p-3.5 sm:rounded-[24px] sm:p-4">
      ...
      <header className="space-y-3">
        <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border/20 pb-3 min-[390px]:pb-3.5 sm:pb-4">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 mb-1.5 flex-wrap">
              <h3 className="clamp-title-mobile font-heading text-lg font-bold text-foreground sm:text-xl" title={row.governorNameRaw || 'Unknown'}>{row.governorNameRaw || 'Unknown'}</h3>
              <StatusPill label={`${Math.round(row.confidence)}% text match`} tone={confidenceTone(row.confidence)} />
            </div>
            
            <div className="flex flex-wrap gap-2 text-sm text-muted-foreground font-medium font-medium">
            ...
            <div className="mt-2.5 flex flex-wrap gap-1.5 text-xs text-muted-foreground opacity-80">
              <span>Row #{row.id.slice(-8)}</span>
              <span>•</span>
              <span>{formatWhen(row.createdAt)}</span>
              <span>•</span>
              <span className={supportedBoard ? 'text-muted-foreground opacity-80' : 'text-rose-400'}>{boardLabel}</span>
            ...
            {candidatePreview.length > 0 && (
              <p className="mt-2.5 text-xs text-muted-foreground opacity-80 bg-muted/10 border border-border/40 p-2 rounded-xl">
                <strong className="text-muted-foreground font-medium block mb-0.5"><Sparkles className="size-3 inline-block mr-1 -mt-0.5 text-cyan-300"/> Known Candidates Matched:</strong> {candidatePreview.join(' • ')}
              </p>
            )}
          </div>
      ...
      <div className="mt-4">
        <section className="space-y-3">
          ...
          <div className="rounded-2xl border border-border/40 bg-muted/10 p-3.5 sm:p-4 lg:p-5">
            <div className="flex items-center gap-2 mb-4 pb-4 border-b border-border/20">
              ...
              >
                <SelectTrigger className="flex-1 rounded-xl border-border/40 bg-white/5 border border-white/5 text-foreground focus:ring-0">
              ...
              <Button
                variant="outline"
                className="shrink-0 rounded-xl border-border/40 bg-white/[0.02] text-foreground hover:bg-muted/10"
                onClick={onRerun}
            ...
            <div className="grid grid-cols-2 gap-2 sm:gap-3">
              <Button
                className="h-12 w-full rounded-2xl bg-primary/20 text-primary hover:bg-primary/20 border border-primary/30 shadow-none text-xs sm:text-sm font-medium transition-all"
          ...
          {rerunHint ? (
            <div className="rounded-2xl border border-border/40 bg-white/5 border border-white/5 p-3.5 text-sm text-muted-foreground font-medium">
              ...
              {Array.isArray(rerunHint.detectedBoardTokens) && rerunHint.detectedBoardTokens.length > 0 ? (
                <p className="mt-2 text-muted-foreground opacity-80">
